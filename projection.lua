...
local function getAnimLength(char, id)
	...
	local animator = getOrCreateAnimator(h)
	if not animator then return 1 end
...
function Projection.new(cfg)
	...
	local self = setmetatable({
		...
		_mirrorGui = nil,
		_mirrorViewport = nil,
		_mirrorWorldModel = nil,
		_mirrorModel = nil,
		_mirrorTrack = nil,
		_mirrorAnimId = nil,
		_hideConn = nil,
		_hiddenCharParts = nil,
	}, Projection)
...
function Projection:_buildTemplate()
	...
	getOrCreateAnimator(h)

	local hrpClone = t:FindFirstChild("HumanoidRootPart")
	if hrpClone then
		t.PrimaryPart = hrpClone
	end
...
function Projection:_buildMirrorTemplate()
	local char = self.Player.Character
	if not char then return nil end
	local t = safeCloneCharacter(char)
	if not t then return nil end
	...
	local toDestroy = {}
	for _, v in t:GetDescendants() do
		if v:IsA("BaseScript") or viewportDestroyClasses[v.ClassName] then
			toDestroy[#toDestroy + 1] = v
		end
	end
	for i = #toDestroy, 1, -1 do toDestroy[i]:Destroy() end
...
function Projection:_updateMirror(pivotCF, animId, timePos)
	...
	if animId and animId ~= 0 then
		if animId ~= self._mirrorAnimId then
			...
			local animator = self:_getAnimator(model)
			if animator then
				self._mirrorTrack = animator:LoadAnimation(getAnim(animId))
				self._mirrorTrack:Play(0, 1, 0)
				self._mirrorAnimId = animId
			end
		end
		if self._mirrorTrack then
			if not self._mirrorTrack.IsPlaying then
				self._mirrorTrack:Play(0, 1, 0)
			end
			self._mirrorTrack:AdjustSpeed(0)
			self._mirrorTrack.TimePosition = timePos or 0
		end
	end
end
...
function Projection:_createMirrorViewport()
	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "ProjectionMirror"
	screenGui.ResetOnSpawn = false
	screenGui.IgnoreGuiInset = true
	screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	screenGui.DisplayOrder = 10
	...
	local worldModel = Instance.new("WorldModel")
	worldModel.Name = "MirrorWorldModel"
	worldModel.Parent = viewport

	screenGui.Parent = self.Player.PlayerGui
	self._mirrorGui = screenGui
	self._mirrorViewport = viewport
	self._mirrorWorldModel = worldModel
end

function Projection:_hideCharacter()
	local char = self.Player.Character
	...
	self._hiddenCharParts = parts

	self._hideConn = RunService.RenderStepped:Connect(function()
		local cached = self._hiddenCharParts
...
function Projection:_showCharacter()
	if self._hideConn then
		self._hideConn:Disconnect()
		self._hideConn = nil
	end
	if self._hiddenCharParts then
		for _, part in self._hiddenCharParts do
			if part and part.Parent then
				part.LocalTransparencyModifier = 0
			end
		end
		self._hiddenCharParts = nil
	end
end

function Projection:_cleanupMirror()
	self:_showCharacter()

	if self._mirrorTrack then
		pcall(self._mirrorTrack.Stop, self._mirrorTrack)
		pcall(self._mirrorTrack.Destroy, self._mirrorTrack)
		self._mirrorTrack, self._mirrorAnimId = nil, nil
	end

	if self._mirrorModel and self._mirrorModel.Parent then
		self._mirrorModel:Destroy()
	end
	self._mirrorModel = nil

	if self._mirrorGui and self._mirrorGui.Parent then
		self._mirrorGui:Destroy()
	end
	self._mirrorGui, self._mirrorViewport, self._mirrorWorldModel = nil, nil, nil
end
	...
	if track then
		...
		for _, desc in model:GetDescendants() do
			if desc:IsA("Motor6D") and desc.Part0 and desc.Part1 then
				local p0 = desc.Part0
				if not motorsByPart0[p0] then
					motorsByPart0[p0] = {}
				end
				motorsByPart0[p0][#motorsByPart0[p0] + 1] = desc
			end
		end
		...
		if root then
			...
			while head <= #queue do
				...
				head += 1
				local motors = motorsByPart0[current]
				if motors then
					for _, motor in motors do
						local p1 = motor.Part1
						if p1 then
							p1.CFrame = current.CFrame * motor.C0 * motor.Transform * motor.C1:Inverse()
							queue[#queue + 1] = p1
						end
					end
				end
...
function Projection:SetMirrorMode(enabled) self.MirrorMode = enabled return self end
...
function Projection:Play(callback)
	...
	if mirrorMode then
		...
		if mirrorTemplate then
			self:_createMirrorViewport()
			self._mirrorModel = mirrorTemplate
			mirrorTemplate.Parent = self._mirrorWorldModel
			local initAnim = animData[1]
			self:_updateMirror(pivots[1], initAnim[1], initAnim[2])
			self:_hideCharacter()
		else
			mirrorMode = false
		end
	...
	local function createProjection(frameIdx)
		...
		if pooled then
			...
			local cloneHRP = p:FindFirstChild("HumanoidRootPart")
			if cloneHRP then
				p.PrimaryPart = cloneHRP
			end
	...
	local function processSettled()
		...
		for slot in pairs(pendingSetup) do
			...
			if entry and not entry.ready and (now - entry.setupTime) >= settleTime then
				if entry.track then
					pcall(entry.track.AdjustSpeed, entry.track, 0)
					pcall(function() entry.track.TimePosition = entry.timePos end)
				end
				if useViewport then
					-- viewport mode: just lock speed, engine renders correct pose
				else
	...
	if self.PlayAnimation then
		local h = char:FindFirstChildWhichIsA("Humanoid")
		local animator = h and getOrCreateAnimator(h)
		if animator then
			for aid in pairs(uniqueAnims) do
				local track = animator:LoadAnimation(getAnim(aid))
	...
	if not instantSettle then
		for _ = 1, max(10, floor(settleTime / 0.016) + 5) do RunService.Heartbeat:Wait() processSettled() end
	end
	...
	conn = RunService.Heartbeat:Connect(function()
		...
		if draining then
			...
			if mirrorMode then
				local endAnim = animData[n]
				self:_updateMirror(pivots[n], endAnim[1], endAnim[2])
			end
			if drainIdx > n then
				conn:Disconnect()
				self._conn = nil
		...
		if mirrorMode then
			...
			self:_updateMirror(mirrorPivot, curAnim[1], mTimePos)
		end
